// ============================================================
// test_refine_ft8_delay_v8_2_1.cpp
//
...
static float refine_ft8_delay_v8_2(
    const float* samples,
    int num_samples,
    const uint8_t* tones,
    float delay0,
    ...
    RefineTiming* timing = nullptr)
{
    ...
    // --------------------------------------------------------
    // COARSE SEARCH - sync symbols only
    // --------------------------------------------------------
    const uint64_t t_coarse0 = bench_now_us();
    // For the coarse acquisition use only the known FT8 Costas/sync
    // regions. In this benchmark the complete tone sequence is known,
    // so we use the first, middle and last 3 symbols. This reduces the
    // expensive coarse work from 79 symbols to 9 symbols while keeping
    // the three time-separated regions of the FT8 frame.
    static constexpr uint8_t SYNC_SYMBOLS[9] =
        { 0, 1, 2, 36, 37, 38, 75, 76, 77 };
...
static void run_one_test(const char* filename, float true_delay)
{
    ...
    const float estimated_delay =
        refine_ft8_delay_v8_2(samples, num_samples, tones,
...
static void run_all_tests()
{
    printf("\r\n\r\n");
    printf("############################################################\n");
    printf(" FT8 refine_ft8_delay_v8_2 - REAL WAV BENCHMARK\n");
    printf("############################################################\n");
